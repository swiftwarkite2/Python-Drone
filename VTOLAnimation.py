...
class VTOLAnimation:
    def __init__(self):
        ...
        #plt.plot([-P.length/5, P.length], [0.0, 0.0], 'k')    # Draw a base line        #plt.axis([-P.length / 5, P.length + P.length / 5, -P.length/5, P.length+P.length/5])  # Change the x,y axis limits
        #plt.axis([-P.length/5, P.length + P.length / 5, -P.length/5, P.length+P.length/5])  # Change the x,y axis limits
        plt.plot([-P.length, P.length], [0.0, 0.0], 'k')    # Draw a base line        #plt.axis([-P.length / 5, P.length + P.length / 5, -P.length/5, P.length+P.length/5])  # Change the x,y axis limits
        plt.axis([-P.length * 1.2, P.length + P.length / 5, -P.length * 1.2, P.length+P.length/5])  # Change the x,y axis limits
